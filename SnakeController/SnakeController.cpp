      ...
      m_paused(false)
{
    ...
    Dimension _dimension_m_world;
    _dimension_m_world.width=width;    
    _dimension_m_world.height=height;
    Position food_m_world;
    food_m_world.x=foodX;
    food_m_world.y=foodY;

    if (w == 'W' and f == 'F' and s == 'S') {
        m_world = std::make_unique<World>(_dimension_m_world, food_m_world);
        ...
        while (length--) {
            int x, y;
            istr >> x >> y;
            Position positionTempContainer{x, y};
            m_segments->addSegment(positionTempContainer);
        }
    } else {
        throw ConfigurationError();
    }
...
void Controller::sendClearOldFood()
{
    ...
    Position position=clearOldFood.position;
...
void Controller::removeTailSegment()
{
    ...
    Position position=clearTail.position;
    position=tail;
